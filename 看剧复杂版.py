...
def get_first_m3u8_url(url):
    ...
    resp.encoding = 'utf-8'
    resp.close()
    obj=re.compile(r'RESOLUTION=720x406(?P<first_m3u8_url>.*)', re.S)
...
async def aio_download():
    ...
    async with aiohttp.ClientSession() as session:
        async with aiofiles.open('1.txt',mode='r',encoding='utf-8') as file:
...
def main(url):
    ...
    # 3.根据真实m3u8的地址下载保存
    download_m3u8_file(real_first_m3u8_url,'1.txt')
    # 4.下载ts片段视频
...
if __name__ == '__main__':
    url='http://www.wwmulu.com/rj/qyfddf/play-1-8.html'
    main(url)